...
**DD-3b — the fair `<use>` comparison (2026-08-31, external review demand, measured)**: the reviewer correctly noted the original large-file comparison was apples-to-oranges — svb templated the repetition while the SVG side stayed flat. Rerun with the SVG side using its native optimization (`<defs>` + 12,100 `<use>` instances + svgo + brotli): **13,509 B vs svb+brotli 1,443 B — svb remains 9.4× smaller**. Mechanism: each text `<use>` instance costs ~1.1 B even after LZ+entropy (the varying coordinates are literals), while a delta-chained binary instance costs ~0.12 B. The reviewer's further prediction (SVG+use would drop to ~1 KB) is falsified. Consequence adopted: v0.3 adds `<use>` *ingestion* to the encoder (parse SVG-native use → template + instance), so already-templated production SVGs encode natively; today the encoder skips `<use>` elements (documented limitation). Script: `benchmark/use-test.mjs`.
